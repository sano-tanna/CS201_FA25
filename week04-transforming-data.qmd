...
```{r, message=FALSE}
penguins_male_female <- penguins %>% filter(sex %in% c("Male","Female"))

p <- ggplot(penguins_mf, aes(bill_length_mm, bill_depth_mm, color = species)) +
  geom_point() +
...
2.  Now, create a bar graph using the *new* dataframe you made
    ...
    ```{r}
    ggplot(average_mass, aes(x = species, y = average_body_mass, fill = species)) +
      geom_col(width = 0.7) + # same as geom_bar(stat = "species")
...
```{r, message=FALSE}
ggplot(average_flipper_length, 
       aes(x = species, y = average_flipper_length, fill = species)) +
  geom_bar(stat = "identity", width = 0.3) +
  theme_bw() +
  coord_flip() + 
  theme(legend.position = "none") +
  labs(title = "Average Flipper Length by Species",
       x = "Species",
...
```{r, message=FALSE}
bp <- ggplot(penguins_data, aes(x = species, y = body_mass_g)) + geom_boxplot()
ggplotly(bp)

ggplot(data = penguins_data, aes(x = species, y = body_mass_g, fill = species)) + geom_boxplot()
ggplotly(ex2)
...
```{r, message=FALSE}
culmen_depth_mm <- penguins_data %>%
  group_by(species) %>%
  summarise(average_culmen_depth = mean(culmen_depth_mm, na.rm = TRUE))

ggplot(culmen_depth_mm,
       aes(x = species, y = average_culmen_depth, fill = species)) +
  geom_bar(stat = "identity", width = 0.3) +
  labs(title = "Average Culmen Depth by Species",
       x = "Species",
