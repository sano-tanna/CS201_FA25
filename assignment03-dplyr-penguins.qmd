...
```{r, message=FALSE, warning=FALSE}
#1: keep only females on Dream or Biscoe with known flipper length
fb1 <- penguins %>%
  dplyr::filter(
    sex == "FEMALE",
...
#2: keep just the required columns
fb2 <- fb1 %>%
  dplyr::select(species, island, flipper_length_mm, body_mass_g)

#3: make the flipper_rank with case_when()
fb3 <- fb2 %>%
  dplyr::mutate(flipper_rank = dplyr::case_when(
    flipper_length_mm < 190              ~ "short",
    flipper_length_mm <= 210             ~ "medium",  # (implies >= 190 & <= 210)
    flipper_length_mm > 210              ~ "long"
  ))
#4: summarize average body mass (rounded), then arrange
final_boss <- fb3 %>%
  dplyr::group_by(species, island, flipper_rank) %>%
  dplyr::summarize(
    avg_mass_g = round(mean(body_mass_g, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  dplyr::arrange(species, island, flipper_rank)

# show the result (should be 8 rows x 4 columns)
print(final_boss)
