...
```{r}
# Two differences:
# 1) Column name: percent_completed_by_state uses stfips, states_map uses state_fips
# 2) Type/format: stfips is numeric; state_fips is a zero-padded 2-character string
...
```{r}
library(stringr)
# Make a matching join key in your summary table
percent_completed_by_state <- percent_completed_by_state %>%
  mutate(state_fips = str_pad(as.character(stfips), width = 2, pad = "0"))

# (states_map$state_fips is already character; keep as-is)
...
completed_treatment_map_int <- ggplot(percent_completed_by_state_map) +
  geom_sf_interactive(
    aes(
      ...
      fill = percentage_completed,
      tooltip = paste0(
        state_name, ": ", round(percentage_completed, 2), "%"
      )
    ),
